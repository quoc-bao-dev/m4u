...
const SubmitReview = ({ id }: { id: number }) => {
  ...
  const currentProductReview = listProductReview?.find(
  ...
  useEffect(() => {
    if (currentProductReview) {
      ...
      // Lưu hình ảnh từ API
      if (
        currentProductReview.media_other &&
        currentProductReview.media_other.length > 0
      ) {
        const filteredImages = currentProductReview.media_other.filter(
          (media: any) => !imagesToDelete.includes(media.id)
        )
  ...
  const handleVideoRemove = () => {
    ...
    if (
      currentProductReview?.video_review &&
  ...
  const overallExperienceItems = Array.isArray(typeEvaluate)
    ...
    : []
  const activeColor =
    (listProductReview || []).find((p: any) => p.id === selectedProductId)
      ?.background_color || '#FE6BBA'
  ...
  const buildDefaultRatings = (): Record<number, number> => {
    ...
    typeEvaluate.forEach((item: any) => {
      ...
      const existingRating = currentProductReview?.list_evaluate?.find(
        (evaluation: any) => evaluation.id_evaluate === item.id
      )?.star
  ...
  return (
    <div className="pt-[72px] relative overflow-hidden">
      <Image
        ...
        height={600}
        alt="top-gradient"
        className="absolute z-[-1] top-0 left-0 w-full object-cover -translate-y-1/3 scale-x-[-1.3] opacity-60"
      />
              ...
              return (
                ...
                >
                  {isActive && (
                    <>
                      <CornerTag
                        className="absolute top-0 right-0 w-[25px] lg:w-[50px]"
                        style={{ color: item.background_color || '#FE6BBA' }}
                      />
                  ...
                  <div
                    ...
                    style={{
                      borderColor: isActive
                        ? withAlpha(item.background_color || '#FE6BBA', 0.9)
                        : 'transparent',
                      backgroundColor: withAlpha(
                        item.background_color || '#FE6BBA',
                        0.2
                      ),
                    }}
                  >
                    <div className="p-0.5 lg:p-1 px-7 lg:px-10 2xl:px-20 rounded-md xl:rounded-xl bg-white w-full aspect-[209/221] flex items-center justify-center">
                      <Image
                        src={item?.image || IMAGES.imageNoData}
                        alt="review"
                        width={1000}
                        height={1000}
                    ...
                    <div className="flex flex-col gap-0.5 lg:gap-1">
                      <h3 className="text-[6px] lg:text-[10px] font-bold text-greyscale-900">
                        {item?.code}
                      </h3>
                      <p className="text-[8px] lg:text-sm font-normal text-greyscale-900">
                        {item?.name}
                      </p>
                    </div>
                  </div>
        ...
        <div className="lg:flex-[915] h-fit p-0 lg:p-4 xl:p-10 rounded-none lg:rounded-3xl flex flex-col gap-4 xl:gap-6 bg-transparent lg:bg-white shadow-none lg:shadow-[0px_0px_60px_0px_rgba(0,0,0,0.1)]">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              ></span>
              <h2 className="text-xl font-semibold text-greyscale-900">
                {t('shareReview')}
              </h2>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            >
              ...
              >
                ...
                <p className="text-xs font-normal text-greyscale-900">
                  {selectedImages.length + existingImages.length}/8
                </p>
              </div>
              ...
              {existingImages.map((media: any) => (
                <div
                  ...
                  className="relative size-[72px] rounded-lg overflow-hidden"
                >
                  {currentProductReview?.active === 0 && (
                    <div
          ...
          <div className="flex flex-col lg:flex-row justify-between gap-6">
            <div className="flex flex-col gap-4">
              ...
              {(productQualityItems.length ? productQualityItems : []).map(
                (item) => {
                  // Nếu active = 0, cho phép chỉnh sửa, ngược lại hiển thị từ API
                  const existingRating =
                    currentProductReview?.list_evaluate?.find(
                      (evaluation: any) => evaluation.id_evaluate === item.id
                    )?.star
                  const currentRating =
                    currentProductReview?.active === 0
                  ...
                  return (
                    <div key={item.id} className="flex items-center gap-2">
                      <Rating
                        ...
                        onChange={
                          currentProductReview?.active === 0
                            ? (v) => handleChangeRating(item.id, v)
                            : undefined
                        }
                        maxWidth={136}
                      />
                      <p className="text-sm font-normal text-[#4E5969]">
                        {item.name}
                      </p>
                    </div>
                  )
            ...
            <div className="flex flex-col gap-4">
              ...
              {(overallExperienceItems.length
                ? overallExperienceItems
                : []
              ).map((item) => {
                // Nếu active = 0, cho phép chỉnh sửa, ngược lại hiển thị từ API
                const existingRating =
          ...
          <div className="flex flex-col gap-3">
            <div className="flex justify-between gap-2">
              <h3 className="text-sm font-semibold text-[#4E5969]">
                {t('tellUsWhatYouThink')}
              </h3>
              <div className="flex items-center gap-2 group cursor-pointer">
                <LightbulbIcon className="size-4 text-[#3B82F6]" />
                <p className="text-sm font-normal text-[#3B82F6] group-hover:underline">
                  {t('reviewTips')}
                </p>
              </div>
            </div>
