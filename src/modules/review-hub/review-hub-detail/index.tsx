...
import { useGetDataReviewHubDetailInfinite, useGetProductRelationListReviewHub } from '@/services/review-hub/queries'
...
const ReviewHubDetail = ({ slug }: { slug: string }) => {
  ...
  const { isLoading: isLoadingSimilar, data: productRelationListReviewHub, isError } = useGetProductRelationListReviewHub({ id: data?.pages?.[0]?.id })
