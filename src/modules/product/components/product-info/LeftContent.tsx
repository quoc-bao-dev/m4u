...
const LeftContent = () => {
  ...
  const thumbnails = [0, 1, 2, 3, 0, 0]

  const scrollToIndex = (index: number) => {
    if (scrollRef.current) {
      const itemWidth = 120 + 8 // width + gap
  ...
  return (
    <div className="md:sticky md:top-10 self-start">
      <div className="relative aspect-[6/8] w-full rounded-2xl overflow-hidden">
        <Image
      ...
      <div className="relative mt-2">
        ...
        >
          {thumbnails.map((val, index) => (
            ...
            >
              <img
                src={`/image/product/image-slide-0${val}.png`}
                alt={`product-thumb-${val}`}
                className={`w-[120px] h-[120px] object-cover rounded-lg transition-all duration-200 ${
                  index === currentIndex
...
const LinkedinIcon = () => {
  return (
    <svg
      ...
      xmlns="http://www.w3.org/2000/svg"
    >
      <g clipPath="url(#clip0_18353_23140)">
        <path
