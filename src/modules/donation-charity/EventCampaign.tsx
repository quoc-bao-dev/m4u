...
const EventCampaign = () => {
  const t = useTranslations('donationCharity.eventCampaign')
  const { hasEvent } = useDonationCharityStore()
  const { data: donationsAndCharity, isLoading } = useGetDonationsAndCharity()

  const content = useMemo(() => {
    return donationsAndCharity?.data?.section3
  }, [donationsAndCharity])
  ...
  return (
    <div className="py-8 xl:py-24 pl-3 md:pl-0 flex flex-col items-center justify-center w-full">
      <div className="flex flex-col items-center justify-center gap-4 container-custom">
        <h2
          className="pr-3 md:pr-0 text-2xl xl:text-6xl font-bold"
        ...
        <p
          className="pr-3 md:pr-0 text-base xl:text-2xl text-greyscale-700 text-center max-w-6xl"
      ...
      <div className="flex flex-col md:pl-6 lg:pl-12 xl:pl-24 w-full">
        ...
        {!hasEvent && <div className="pt-[96px]"> </div>}
