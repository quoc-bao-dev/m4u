...
> => ({
  happening: {
    label: tr('status.happening'),
    className: 'bg-red-600',
    showIcon: true,
  },
  coming: {
    label: tr('status.coming'),
    className: 'bg-[#2DD4BF]',
    showIcon: false,
  },
})
...
const EventCard = ({
  ...
  const t = useTranslations('donationCharity.eventCard')
    ...
    <div className="group flex flex-col gap-2 xl:gap-4 shadow-[0px_4px_10px_0px_#00000040] hover:shadow-xl transition-all duration-300 rounded-xl pb-2 xl:pb-4 cursor-pointer">
      ...
      <div className="px-2 xl:px-4 flex flex-col gap-4">
        <div className="flex items-center justify-between gap-4">
          <div
            className={`flex items-center gap-1 rounded-full py-1 lg:py-2 px-2 lg:px-3 ${cfg.className}`}
          >
            {cfg.showIcon && (
              <BroadcastIcon className="text-white size-3 lg:size-4" />
            )}
            <span className="text-xs font-medium text-white">{cfg.label}</span>
          </div>
          <p className="text-xs lg:text-base font-normal text-greyscale-700">
            {date}
          </p>
        </div>
        <h4 className="text-base lg:text-2xl font-bold text-greyscale-900 group-hover:text-pink-500 transition-all duration-300">
          {title}
        </h4>
        <div className="flex items-center gap-3">
          ...
          <p className="text-sm lg:text-base font-medium text-greyscale-700">
            {t('fundName')}
          </p>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div className="flex flex-col gap-1">
            <p className="text-[10px] lg:text-sm font-normal text-greyscale-700">
              {t('totalProducts')}
            </p>
            <p className="text-sm lg:text-2xl font-semibold text-orange-500">
              {productCount}
            </p>
          </div>
          <div className="flex flex-col gap-1">
