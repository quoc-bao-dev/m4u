...
const ProductCard = ({
  ...
  return (
    ...
    >
      ...
      {variant === 'main' ? (
        <div
          className={`relative z-10 w-[90%] aspect-[265/298] will-change-transform transform-gpu transition-transform duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${
            isEnteringTop ? 'translate-y-32' : 'translate-y-20'
          }`}
        >
          <PolygonBg
            className={`absolute inset-0 pointer-events-none`}
            style={{ color: colors.polygon }}
          />
          <Image
            src={imageSrc}
            alt={productName}
      ...
      >
        ...
        <div
          className={`p-1.5 lg:p-4 flex flex-col items-center justify-center ${
            variant === 'main' ? 'pt-28 lg:pt-36' : 'pt-4'
          }`}
        >
          <h3
            className={`text-[8px] lg:text-base font-bold text-greyscale-900`}
          >
            {brand}
          </h3>
          <p
            className={`h-10 lg:h-16 text-xs lg:text-2xl font-medium text-greyscale-700 text-center line-clamp-2`}
          >
            {productName}
          </p>
        </div>
        ...
        >
          <p className={`text-[8px] lg:text-xl text-greyscale-700 text-center`}>
            ...
            {useTranslations('donationCharity.productCard')('contribution')}
          </p>
        </div>
      </div>
    </div>
