    ...
    <DayPicker
      ...
      components={{
        ...
        Weekday: ({ children, ...props }) => {
          // Chỉ áp dụng màu đỏ cho "CN" (Chủ nhật tiếng Việt), không áp dụng cho "SU" (Sunday)
          const isVietnameseSunday = children === 'Su'
          return (
            <th {...props}>
              <div
                className={cn(
                  'text-muted-foreground rounded-md flex-1 font-normal text-[0.8rem] select-none',
                  isVietnameseSunday && 'text-red-500'
                )}
              >
                {children}
              </div>
...
function CalendarDayButton({
  ...
  const defaultClassNames = getDefaultClassNames()
  const isSunday = day.date.getDay() === 0 // 0 = Chủ nhật
