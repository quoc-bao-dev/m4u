...
const UserAvatar: React.FC<UserAvatarProps> = ({
  ...
  if (src && !imgError) {
    return (
      <div
        className={`h-full rounded-full overflow-hidden border-2 border-greyscale-200 ${
          className ?? ''
        }`.trim()}
        // style={{ width: size, height: size }}
      >
  ...
  return (
    <div
      className={`rounded-full flex items-center justify-center text-white text-sm font-semibold ${
        className ?? 'w-full h-full border-2 border-greyscale-200'
      }`.trim()}
